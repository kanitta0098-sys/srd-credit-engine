...
st.set_page_config(page_title="SRD Hybrid v2.8.1.1 - Fix เมนู API", layout="wide", page_icon="🏍️")
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Sarabun:wght@400;600;700;800&display=swap');
html, body, [class*="css"] { font-family:'Sarabun', sans-serif !important; }
.stApp { background:#0F172A !important; color:#E2E8F0 !important; }
/* FIX แถบดำปิดหัวแอป - แก้ไม่ให้เมนูหาย */
header[data-testid="stHeader"] {background:transparent !important; height:40px !important;}
div[data-testid="stToolbar"] {visibility:hidden !important; display:none !important;}
div[data-testid="stHeaderActionElements"] {display:none !important; visibility:hidden !important;}
#MainMenu {visibility:hidden !important; display:none !important;}
footer {visibility:hidden !important; display:none !important;}
/* คงปุ่มเปิด Sidebar ไว้ */
...
# SIDEBAR - คงเดิม
with st.sidebar:
    st.markdown("### 🏍️ SRD Hybrid v2.8.1\nปัดเศษ + กระชับ")
                ...
                if 'model_sel' not in locals() or not model_sel:
                    for m in genai.list_models():
                        if 'generateContent' in m.supported_generation_methods:
                            ...
                            break
                st.success(f"✅ API Key ใช้งานได้ - {model_sel if 'model_sel' in locals() else 'พร้อม'}")
            except Exception as e:
                st.error(f"API Key Error: {e}")
        else:
            st.warning("⚠️ กรุณากดปุ่ม ☰ มุมซ้ายบนเพื่อเปิด Sidebar แล้วกรอก API Key ในเมนู หรือกรอกตรงนี้")
    ...
    if st.button("🚀 รัน SRD Credit Investigation Engine 13 Modules + Prompt AI 10 หัวข้อ", type="primary", use_container_width=True, key="run_ai_v28"):
        if not api_key or not model_sel:
            ...
            try:
                ...
                full_prompt=f"""
# SRD Hybrid v2.8.1.1 - Fix เมนู API
